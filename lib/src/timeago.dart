...
String format(
  ...
  return [if (getPrefixSuffix) prefix, result, if (getPrefixSuffix) suffix].where((str) => str.isNotEmpty).join(messages.wordSeparator());
}
...
extension DATETIMECHECK on DateTime {
  ...
  bool checkExpireDate({int number = 6, String validate = 'days'}) {
    var ago = format(this, getPrefixSuffix: false);

    var s = ago.split(' ').first;

    var num = int.parse((s == 'a' || s == 'about') ? '1' : s);
    var m = ago.split(' ').last;
    ...
    if (validate == 'minutes') {
      ...
      if (number == 1 && ago == 'about a month') {
        ...
        if (m == 'months') {
          ...
          return ago == 'about a year' || m == 'years';
        }
      }
